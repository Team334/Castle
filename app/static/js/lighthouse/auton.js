...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Add flip and rotate button listeners
...
function initCanvas() {
  try {
    ...
    if (typeof Canvas !== "function") {
      console.error(
        "Canvas class not found! Make sure Canvas.js is loaded properly.",
      );
      return;
    }
...
function updateTeamInfo(data) {
  ...
  const location = [info.city, info.state_prov, info.country]
...
function addPathToSelection(index) {
  ...
  const allianceCount = selectedPaths.filter(
    (p) => p.alliance === alliance,
  ).length;

  // Check if we've reached the per-alliance limit
  if (allianceCount >= MAX_PER_ALLIANCE) {
    alert(
      `Cannot add more than ${MAX_PER_ALLIANCE} teams from the ${alliance} alliance`,
    );
    return;
  }
...
function updateSelectedPaths() {
  ...
  const existingCards = selectedPathsContainer.querySelectorAll(
    ".selected-path-card",
  );
...
function drawPaths() {
  ...
  try {
      ...
      try {
        ...
        if (typeof pathToDraw === "string") {
          try {
            ...
            const sanitizedValue = pathToDraw
              .trim()
              .replace(/&quot;/g, '"')
              .replace(/&#34;/g, '"')
              .replace(/&#39;/g, "'")
            ...
            console.error(
              `Error parsing path data for path ${pathIndex}:`,
              parseError,
            );
            return;
          }
        ...
        if (!Array.isArray(pathToDraw)) {
          console.warn(
            `Path data is not an array for path ${pathIndex}:`,
            pathToDraw,
          );
          return;
        }
        ...
        console.log(
          `Processing path ${pathIndex} with color ${path.color}, ${pathToDraw.length} strokes`,
        );
        ...
        console.error(
          `General error processing path ${pathIndex}:`,
          pathError,
          path,
        );
      }
    ...
    try {
      ...
      console.log(
        `Successfully drew ${combinedDrawingHistory.length} strokes from ${selectedPaths.length} paths`,
      );
    } catch (drawError) {
