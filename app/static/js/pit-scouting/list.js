function exportToCSV() {
  ...
  rows.forEach((row) => {
    try {
      // Team Number
      const teamNumber = row
        .querySelector("td:nth-child(1) .text-lg")
        .textContent.trim();

      // Drive Type & Swerve Modules
      const driveTypeCell = row.querySelector("td:nth-child(2)");
      const driveTypes = Array.from(
        driveTypeCell.querySelectorAll(".rounded-full"),
      )
      ...
      // Programming & Auto
      const progCell = row.querySelector("td:nth-child(5)");
      const progText = progCell.textContent.trim();
      const programmingLang = (progText.match(
        /Programming Language: (.+?)(?:\n|$)/,
      ) || ["", ""])[1].trim();
      const hasAuto = progText.includes("✅");
      const autoRoutes = (progText.match(/(\d+) routes/) || ["", ""])[1];
      const preferredStart = (progText.match(
        /Preferred Start: (.+?)(?:\n|$)/,
      ) || ["", ""])[1];
      ...
      // Driver Experience
      const driverCell = row.querySelector("td:nth-child(6)");
  ...
  searchInput.addEventListener("input", function () {
    ...
    tableRows.forEach((row) => {
      ...
      if (teamNumberCell) {
        ...
        row.style.display =
          searchTerm === "" || teamNumberText.includes(searchTerm)
            ? ""
            : "none";
      }
    });
